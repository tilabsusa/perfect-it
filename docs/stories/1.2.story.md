...
## Tasks / Subtasks
- [ ] **Task 1: Enhance Cognito Configuration** (AC: 1, 2, 3, 4)
  - [ ] Update `amplify/auth/resource.ts` to include social providers (Google, Facebook)
  - [ ] Configure required user attributes: email (required), username (required), preferred_username
  - [ ] Set password policy: minimum 8 chars, require numbers and special characters
  - [ ] Enable email verification for new registrations
  - [ ] Test configuration with `amplify sandbox` deployment

- [ ] **Task 2: Configure OAuth Providers** (AC: 1, 5)
  - [ ] Set up Google OAuth in Google Cloud Console and obtain credentials
  - [ ] Set up Facebook OAuth in Facebook Developer Console and obtain App ID/Secret
  - [ ] Add OAuth provider configurations to `amplify/auth/resource.ts`
  - [ ] Configure redirect URLs and OAuth scopes
  - [ ] Test OAuth flows in sandbox environment

- [ ] **Task 3: Define User Groups and Roles** (AC: 6)
  - [ ] Create Cognito user groups: standard_user, moderator, admin
  - [ ] Configure group permissions and precedence values
  - [ ] Add group definitions to `amplify/auth/resource.ts`
  - [ ] Document role-based access control strategy

- [ ] **Task 4: Update GraphQL Schema with User Model** (AC: 7)
  - [ ] Update `amplify/data/resource.ts` with User model from architecture
  - [ ] Add required fields: username, email, avatarUrl, bio, expertiseTags, reputationScore, isVerified, socialLinks
  - [ ] Configure authorization rules: owner access, authenticated read, guest read for public data
  - [ ] Add secondary index on username field for lookups
  - [ ] Define relationships: hasMany for cards, comments, votes, collections

- [ ] **Task 5: Create Post-Confirmation Lambda Trigger** (AC: 8)
  - [ ] Create `amplify/functions/post-confirmation/` directory structure
  - [ ] Implement `handler.ts` to create User record in DynamoDB after signup
  - [ ] Map Cognito sub to User.id in the database
  - [ ] Initialize default values: reputationScore=0, isVerified=false
  - [ ] Configure Lambda trigger in `amplify/auth/resource.ts`
  - [ ] Add package.json and tsconfig.json for Lambda function

- [ ] **Task 6: Update Backend Configuration** (AC: 1, 7)
  - [ ] Update `amplify/backend.ts` to include enhanced auth configuration
  - [ ] Set defaultAuthorizationMode to 'userPool' in data configuration
  - [ ] Configure API key authorization mode with 30-day expiration
  - [ ] Ensure all resources are properly linked

- [ ] **Task 7: Configure Multi-Factor Authentication** (AC: 3)
  - [ ] Enable optional MFA in Cognito configuration
  - [ ] Support both SMS and TOTP authenticator apps
  - [ ] Configure MFA enforcement for admin group
  - [ ] Document MFA setup process for users

- [ ] **Task 8: Write Unit Tests** (AC: 8)
  - [ ] Create `amplify/functions/post-confirmation/handler.test.ts`
  - [ ] Test User record creation with correct attributes
  - [ ] Test error handling for database failures
  - [ ] Verify Lambda function has proper IAM permissions
  - [ ] Achieve 85% code coverage for Lambda function [Source: architecture/testing-strategy.md]
